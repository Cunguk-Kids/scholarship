...
export const faucetController = async (c: Context) => {
  ...
  const [faucet] = await db
    ...
    .from(faucetData)
    .where(and(
      eq(faucetData.address, address),
      eq(faucetData.ipAddress, ipAddress),
    ));
      ...
      return c.json(
        {
          ...
          nextClaimAt: new Date(faucet.updatedAt.getTime() + 24 * 60 * 60 * 1000),
        },
        400
      );
    }
  ...
  await db.insert(faucetData).values({
