...
contract ScholarshipCore is ScholarshipCoreBase {
    ...
    function initialize(address _admin, address _usdc, address _treasury, address _reputation, address _donorNFT, address _studentNFT) external initializer {
        _initReentrancy();
        admin = _admin;
    ...
    function createProgram(
        ...
        uint8 maxCandidates, uint8 targetWinners,
        ...
        programs[pid] = ScholarshipTypes.Program({ id: pid, initiator: msg.sender, metadataCID: metadataCID, educationLevel: educationLevel, screeningMode: screeningMode, status: ScholarshipTypes.ProgramStatus.CREATED, scoreWeights: weights, slashDist: slashDist, maxCandidates: maxCandidates, targetWinners: targetWinners, applicationStart: timeline[0], applicationEnd: timeline[1], votingStart: timeline[2], votingEnd: timeline[3], milestoneDisputeWindow: dw, totalFund: totalFund, allocatedFund: 0, spentFund: 0, yieldAccrued: 0, applicantCount: 0, shortlistedCount: 0, activeScholarCount: 0 });

        if (committeeContract != address(0)) { programCommittee[pid] = committeeContract; emit CommitteeAssigned(pid, committeeContract); }
    ...
    function applyToProgram(
        ...
        applicants[programId][msg.sender] = ScholarshipTypes.Applicant({ programId: programId, wallet: msg.sender, status: ScholarshipTypes.ApplicationStatus.PENDING_REVIEW, profileCID: profileCID, documentCID: documentCID, essayCID: essayCID, recommendCID: recommendCID, screeningScore: 0, totalScore: 0, voteScore: 0, scoreTimestamp: 0, retryCount: retry, scoreDisputed: false });
    ...
    {
        if (programs[programId].screeningMode != ScholarshipTypes.ScreeningMode.BY_COMMITTEE) revert InvalidProgramStatus(ScholarshipTypes.ProgramStatus.SCREENING, programs[programId].status);
    ...
    function resolveShortlist(uint256 programId, address[] calldata ranked) external programExists(programId) onlyInitiator(programId) {
        ...
        for (uint256 i = 0; i < n - 1; ) { if (applicants[programId][ranked[i]].screeningScore < applicants[programId][ranked[i+1]].screeningScore) revert InvalidSortOrder(); unchecked{++i;} }

        uint256 ss = n < prog.maxCandidates ? n : prog.maxCandidates;
        for (uint256 i = 0; i < ss; ) { address st = ranked[i]; ScholarshipTypes.Applicant storage app = applicants[programId][st]; if (app.screeningScore >= ScholarshipTypes.SCREENING_THRESHOLD) { app.status = ScholarshipTypes.ApplicationStatus.SHORTLISTED; _shortlist[programId].push(st); prog.shortlistedCount++; emit StudentShortlisted(programId, st, app.screeningScore); } else _markScreenedOut(programId, st); unchecked{++i;} }
    ...
    function cancelProgram(uint256 programId) external nonReentrant programExists(programId) onlyInitiator(programId) {
        ScholarshipTypes.Program storage prog = programs[programId];
        if (prog.status == ScholarshipTypes.ProgramStatus.ACTIVE || prog.status == ScholarshipTypes.ProgramStatus.COMPLETED || prog.status == ScholarshipTypes.ProgramStatus.CANCELLED) revert InvalidProgramStatus(ScholarshipTypes.ProgramStatus.CREATED, prog.status);
