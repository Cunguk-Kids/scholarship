...
export function ApplyPage() {
  ...
  if (isLoading) return <div className="max-w-3xl mx-auto p-8"><NeoSkeleton lines={5} /></div>;
  if (!program) return <div className="p-8 text-center text-red-500 font-bold">Program not found</div>;

  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      <div className="mb-8">
        <Link to={`/programs/${id}`} className="text-skpurple font-bold hover:underline">← Back to Program</Link>
          ...
          {step === "form" && (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-skyellow-light p-4 rounded-xl border-2 border-black">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea required className="w-full border-2 border-black p-2 rounded focus:outline-skpurple" rows={2} value={formData.profileText} onChange={e => setFormData(p => ({ ...p, profileText: e.target.value }))} />
                  </div>
                  <div>
                    <label className="block text-sm font-bold mb-1">Essay Answers</label>
                    <textarea required className="w-full border-2 border-black p-2 rounded focus:outline-skpurple" rows={3} value={formData.essayText} onChange={e => setFormData(p => ({ ...p, essayText: e.target.value }))} />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-skpurple-light p-4 rounded-xl border-2 border-black">
                <h3 className="font-paytone text-lg mb-2">Self-Declared Scores (BY_STUDENT mode)</h3>
                <p className="text-xs text-gray-700 mb-4 bg-white p-2 border border-black rounded">
                  ⚠️ Note: Your scores will be stored on-chain. If anyone spots a false claim, your deposit might be slashed and your application rejected. Be honest!
                </p>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-bold mb-1">Academic (0-100)</label>
                    <input type="number" min="0" max="100" required className="w-full border-2 border-black p-2 rounded focus:outline-skpurple" value={formData.academicScore} onChange={e => setFormData(p => ({ ...p, academicScore: Number(e.target.value) }))} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold mb-1">Income Need (0-100)</label>
                    <input type="number" min="0" max="100" required className="w-full border-2 border-black p-2 rounded focus:outline-skpurple" value={formData.incomeScore} onChange={e => setFormData(p => ({ ...p, incomeScore: Number(e.target.value) }))} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold mb-1">Recommendation</label>
                    <input type="number" min="0" max="100" required className="w-full border-2 border-black p-2 rounded focus:outline-skpurple" value={formData.recommendScore} onChange={e => setFormData(p => ({ ...p, recommendScore: Number(e.target.value) }))} />
                  </div>
                </div>
              </div>
