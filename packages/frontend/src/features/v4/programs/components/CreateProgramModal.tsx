            ...
            <div className="">
              ...
              <div className="grid grid-cols-4 w-full">
                <NeoButton
                  ...
                  onClick={() => setFormData((p) => ({ ...p, educationLevel: 0 }))}>
                  <p className="truncate mask-ellipse w-[70px]">Elementary School</p>
                </NeoButton>
                <NeoButton
                  className={formData.educationLevel == 1 ? 'bg-skpurple-hover' : ''}
                  onClick={() => setFormData((p) => ({ ...p, educationLevel: 1 }))}>
                  <p className="truncate mask-ellipse w-[70px]">Junior High School </p>
                </NeoButton>
                <NeoButton
                  className={formData.educationLevel == 2 ? 'bg-skpurple-hover' : ''}
                  onClick={() => setFormData((p) => ({ ...p, educationLevel: 2 }))}>
                  <p className="truncate mask-ellipse w-[70px]">Senior High School </p>
                </NeoButton>
                <NeoButton
                  className={formData.educationLevel == 3 ? 'bg-skpurple-hover' : ''}
                  onClick={() => setFormData((p) => ({ ...p, educationLevel: 3 }))}>
                  <p className="truncate mask-ellipse w-[70px]">University</p>
                </NeoButton>
              </div>
            </div>
            <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, targetWinners: Number(e.target.value) }))
                  }
                />
              </div>
              <div>
                <label className="block text-sm font-bold mb-1">Max Candidate</label>
                <input
                  type="number"
                  className="w-full border-2 border-black p-2 rounded-lg neo-shadow-sm focus:outline-skpurple"
        ...
        {step === 'approve' && (
          <div className="text-center py-12 space-y-4">
            ...
            <p className="text-sm text-gray-600">
              You need to approve the Treasury contract to transfer {formData.totalFund} USDC.
            </p>
          </div>
        )}
        ...
        <div className="pt-4 border-t-2 border-gray-100 flex justify-end gap-2">
          ...
          {step === 'form' && (
            <NeoButton
              ...
              variant="primary"
              onClick={handleNext}
              disabled={!formData.name || !formData.totalFund}
            />
          )}
          {step === 'approve' && (
            <NeoButton
