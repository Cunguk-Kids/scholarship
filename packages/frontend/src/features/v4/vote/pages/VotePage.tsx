  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      <div className="mb-8">
        ...
        <p className="text-gray-600 text-lg">Use your voting power to select the next generation of scholars.</p>
      </div>
      ...
      {loadingPrograms ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="md:col-span-1 space-y-4">
            <h3 className="font-bold text-sm text-gray-500 uppercase tracking-wide">Active Programs</h3>
            {programs.map(p => (
              <ProgramVoteSelector 
                key={p.id} 
                program={p} 
...
function ProgramVoteSelector({ program, isSelected, onClick }: { program: Program, isSelected: boolean, onClick: () => void }) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end border-b-2 border-black pb-4">
        <div>
          ...
          <p className="text-sm text-gray-600">Select the candidate you believe is most deserving.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {shortlisted.map(applicant => (
          <CandidateCard key={applicant.id} applicant={applicant} programId={program.blockchainId} />
        ))}
      </div>
    </div>
  );
      ...
      <NeoCardBody className="flex flex-col h-full">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-bold text-lg leading-tight">{profile?.fullName ?? "Anonymous Student"}</h3>
              <p className="text-xs text-gray-500 font-mono" title={applicant.wallet}>{applicant.wallet.slice(0,6)}...{applicant.wallet.slice(-4)}</p>
            </div>
          </div>
